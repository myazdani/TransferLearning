...
#y_train = MultiLabelBinarizer().fit_transform(df_train.which_of_these_sentiment_scores_does_the_above_image_fit_into_best)
...
for layer in layers:
    ...
    #lr_pipe = Pipeline([('OVClr', OneVsRestClassifier(LogisticRegression(max_iter=200)))])
    ...
    gs = GridSearchCV(estimator = lr_pipe, param_grid = param_grid, scoring = 'accuracy', cv = 5, n_jobs=-1, error_score=0)
